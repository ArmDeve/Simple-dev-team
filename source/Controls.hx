	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		switch (scheme)
		{
			...
			case Duo(true):
				...
				inline bindKeys(Control.DOWN, [O]);
				...
				inline bindKeys(Control.RESET, [R]);
			case Duo(false):
				inline bindKeys(Control.UP, [FlxKey.F]);
				inline bindKeys(Control.DOWN, [FlxKey.J]);
				inline bindKeys(Control.LEFT, [FlxKey.D]);
				inline bindKeys(Control.RIGHT, [FlxKey.K]);
				inline bindKeys(Control.BACK, [BACKSPACE, ESCAPE]);
				inline bindKeys(Control.BACK, [BACKSPACE, ESCAPE]);
				inline bindKeys(Control.PAUSE, [P, ENTER, ESCAPE]);
				inline bindKeys(Control.RESET, [R]);
	...
	public function loadKeyBinds()
		{
			...
			if (gamepadsAdded.length != 0)
				removeGamepad();
			options.KeyBinds.KeyBinds.keyCheck();
			...
			inline bindKeys(Control.ACCEPT, [Z, SPACE, ENTER]);
			inline bindKeys(Control.BACK, [BACKSPACE, ESCAPE]);
			inline bindKeys(Control.PAUSE, [FlxKey.fromString(FlxG.save.data.pauseBind), ESCAPE]);
		}
